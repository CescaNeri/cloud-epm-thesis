...
%
\Cref{chap:case study} analyzes the case study examined, defining the project requirements and an overview of the methodology used in the study is presented. 
%
...
The Holding Group was born in 2018 as the result of the aggregation of different Entities which belong to the same industry sector, as illustrated in \Cref{fig:structure}
%
...
\Cref{fig:requirements} lists all the technical, business and strategic requirements that should be considered when choosing the right tools and technologies for the implementation of the EPM solution.
%
...
\Cref{fig:cycle} represents the three main stages of Enterprise Performance Management, each of which plays a critical role in managing and improving the performance of a business. 
%
...
\Cref{fig:gartner} displays the Gartner Magic Quadrant for financial planning software published on the 14th December, 2022 \cite{gartner2022magic}.
%
...
As we can see in \Cref{fig:hierarchy}, the time dimension follows a tree-like structure where each level in the hierarchy represents a different level of detail. 
%
...
As we can see in \Cref{fig:dimensions}, the activities of the Holding Group will be evaluated under five different dimensions of analysis: geography, channel, product, brand and Entity.
%
...
%
To populate the dimensions with data, the Holding Group needs to collect financial data from all the Entities as illustrated in \Cref{fig:dataload}.
%
...
\Cref{fig:mapping} illustrates the relationship and the flow of data among various modeling elements:
%
...
%
The integration capabilities of FCCS enable a smooth flow of data to other Oracle EPM solutions, such as Oracle Planning for planning, reporting and forecasting purposes - as shown in \Cref{fig:planning-fccs}.
%
...
%
Enterprise Performance Management leverages advanced technologies to integrate revenue and cost data, providing a holistic view of organizational performance: by analyzing this data, organizations can gain insights into revenue drivers, cost structures, and their interdependencies. 
%
...
\Cref{fig:driver} illustrates the validity of the drivers based on the combinations of Brand, Geography and Channel.
%
...
%
For instance, \Cref{fig:allocation} exhibits two examples of allocations: as we can see, both cases starts from a cost to allocate that has a specific Brand (Brand A), EMEA and E-commerce as a starting combination for the dimensions Brand, Geography and Channel, which amounts to € 1,000.00.
%
...
Regarding the second rule, \Cref{fig:rule-alloc} illustrates the layout of rule from the Calculation Manager, which is the designed tool that allows users to create, manage, and execute complex calculations and business rules within the EPM environment. 
%
...
%
An example of such elements is shown in \Cref{lst:variables}.
%
...
In the next script, the main activity performed is that of clearing the allocation area in order to ensure data integrity and prevent issues like hanging or accumulating values.
%
...
\lstinputlisting[
	...
	float,
	language=CalcScript,
	caption={Possible Drivers for a Specific Combination},
	label={lst:in-combos},
...
%
Here, we can see that the conditions for the driver validity are characterized by a set of Boolean conditions used as denominator, where a number divided by zero is equal to \#Missing
%
The reason for this peculiar notation regards the optimization of the performance of the rule.
%
The Oracle Multidimensional Database Management System evaluates business rules based on the presence or absence of data values and, using \#Missing instead of 0 can have implications for performance, especially when dealing with large datasets. 
%
...
\Cref{fig:icp} represents the intercompany transaction logic requested by the Holding Group.
%
...
Digital Transformation is disrupting businesses in every industry by breaking down barriers between people, businesses and things.
%
By breaking these barriers, they are able to create new products, services and find more efficient
ways of doing business. 
%
These innovations are happening across organizations of all types, sharing a common theme: the ability to transform processes and business models, empower innovation and technology \cite{schwertner2017digital}.
%
...
%
This data-driven approach empowers decision-makers with valuable insights and supports evidence-based decision-making, a core principle of digital transformation. 
%
Enterprise Performance Management can be considered as a key driver in accelerating digital transformation efforts as it streamline and automate critical business processes like budgeting, planning, forecasting, and financial consolidation. 
%
